...
static struct platform_device_id imx_v4l2_devtype[] = {
	{
		.name = "t-v4l2-capture-imx5",
		.driver_data = IMX5_V4L2,
	}, {
		.name = "t-v4l2-capture-imx6",
...
static const struct of_device_id mxc_v4l2_dt_ids[] = {
	{
		.compatible = "fsl,t-imx6q-v4l2-capture",
...
//	os_print("MXC2:%s\n", __func__);
	...
	if (buf->index < 0 || buf->index >= FRAME_NUM) {
		os_print("MXC2:ERROR: v4l2 capture: mxc_v4l2_buffer_status buffers "
		       "not allocated\n");
		return -EINVAL;
	}
...
//	os_print("MXC2:%s\n", __func__);
	...
	if (buf->index < 0 || buf->index >= FRAME_NUM || buf->length <
		...
		os_print("MXC2:ERROR: v4l2 capture: mxc_v4l2_prepare_bufs buffers "
...
//	os_print("MXC2:%s\n", __func__);
	...
	if (NULL == cam) {
		os_print("MXC2:ERROR! cam parameter is NULL\n");
		return -1;
	}

	if (cam->capture_on) {
		os_print("MXC2:ERROR: v4l2 capture: Capture stream has been turned "
		       " on\n");
		return -1;
	}

	if (list_empty(&cam->ready_q)) {
		os_print("MXC2:ERROR: v4l2 capture: mxc_streamon buffer has not been "
			"queued yet\n");
		return -EINVAL;
	}
	if (cam->enc_update_eba &&
		cam->ready_q.prev == cam->ready_q.next) {
		os_print("MXC2:ERROR: v4l2 capture: mxc_streamon buffer need "
...
//	os_print("MXC2:%s: ipu%d/csi%d capture_on=%d %s\n", __func__, cam->ipu_id,
...
//	os_print("MXC2:%s\n", __func__);
	...
	do {
		...
		if (fbi == NULL) {
			os_print("MXC2:ERROR: verify_preview frame buffer NULL.\n");
			return -1;
		}
		...
		if (((strncmp(fbi->fix.id, "DISP3 BG", 8) == 0) &&
			...
			if (ipu_ch == MEM_BG_SYNC) {
				bg_fbi = fbi;
				os_print("MXC2:Found background frame buffer.\n");
			}
		}
	...
	if (foregound_fb) {
		...
		if (win->w.width + win->w.left > bg_fbi->var.xres ||
			...
			os_print("MXC2:ERROR: FG window position exceeds.\n");
			return -1;
		}
	} else {
	...
	if (*width == 0 || *height == 0) {
		os_print("MXC2:Very View ERROR: v4l2 capture: width or height"
			" too small.\n");
		return -EINVAL;
	}

	os_print("MXC2:Very View OK: v4l2 capture: width is %d, height is  %d \n", *width, *height);
	...
	if ((cam->crop_bounds.width / *width > 8) ||
		...
		*width = cam->crop_bounds.width / 8;
		...
		if (*width + win->w.left > width_bound) {
			os_print("MXC2:ERROR: v4l2 capture: width exceeds "
				"resize limit.\n");
			return -1;
		}
		os_print("MXC2:ERROR: v4l2 capture: width exceeds limit. "
			"Resize to %d.\n",
			*width);
	}
	...
	if ((cam->crop_bounds.height / *height > 8) ||
		...
		*height = cam->crop_bounds.height / 8;
		...
		os_print("MXC2:ERROR: v4l2 capture: height exceeds limit "
			"resize to %d.\n",
			*height);
	}
...
//	os_print("MXC2:MVC: start_preview\n");
	...
	os_print("MXC2:End of %s: v2f pix widthxheight %d x %d\n",
		 __func__,
		 cam->v2f.fmt.pix.width, cam->v2f.fmt.pix.height);
	os_print("MXC2:End of %s: crop_bounds widthxheight %d x %d\n",
		 __func__,
		 cam->crop_bounds.width, cam->crop_bounds.height);
	os_print("MXC2:End of %s: crop_defrect widthxheight %d x %d\n",
		 __func__,
		 cam->crop_defrect.width, cam->crop_defrect.height);
	os_print("MXC2:End of %s: crop_current widthxheight %d x %d\n",
		 __func__,
...
//	os_print("MXC2:%s: type=%d\n", __func__, f->type);

	switch (f->type) {
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		os_print("MXC2:   type is V4L2_BUF_TYPE_VIDEO_CAPTURE\n");
		f->fmt.pix = cam->v2f.fmt.pix;
		break;
	case V4L2_BUF_TYPE_VIDEO_OVERLAY:
		os_print("MXC2:   type is V4L2_BUF_TYPE_VIDEO_OVERLAY\n");
		f->fmt.win = cam->win;
		break;
	default:
		os_print("MXC2:   type is invalid\n");
		retval = -EINVAL;
	}
...
//	os_print("MXC2:%s: %dx%d, type=%d\n", __func__, f->fmt.pix.width, f->fmt.pix.height, f->type);

	switch (f->type) {
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		os_print("MXC2:   type=V4L2_BUF_TYPE_VIDEO_CAPTURE\n");
		if (!valid_mode(f->fmt.pix.pixelformat)) {
			os_print("MXC2:ERROR: v4l2 capture: mxc_v4l2_s_fmt: format "
			       "not supported\n");
			return -EINVAL;
		}
		...
		if (strcmp(mxc_capture_inputs[cam->current_input].name,
			...
			os_print("MXC2:s_fmt crop: width is %d, height is  %d \n", cam->crop_current.width, cam->crop_current.height);
		}
//		os_print("MXC2:s_fmt CSI: input is %d, name is  %s \n", cam->current_input, mxc_capture_inputs[cam->current_input].name);
//		os_print("MXC2:s_fmt pix: width is %d, height is  %d \n", f->fmt.pix.width, f->fmt.pix.height);
//		os_print("MXC2:s_fmt cam: width is %d, height is  %d \n", cam->crop_current.width, cam->crop_current.height);
		...
		*height &= ~7;
		...
		if (*width == 0 || *height == 0) {
			os_print("MXC2:s_fmt ERROR: v4l2 capture: width or height"
				" too small.\n");
			return -EINVAL;
		}

//		os_print("MXC2:s_fmt OK: v4l2 capture: width is %d, height is  %d \n", *width, *height);
		...
		break;
	case V4L2_BUF_TYPE_VIDEO_OVERLAY:
		os_print("MXC2:   type=V4L2_BUF_TYPE_VIDEO_OVERLAY\n");
...
//	os_print("MXC2:%s\n", __func__);
	...
	case V4L2_CID_BRIGHTNESS:
		if (cam->sensor) {
			...
			os_print("MXC2:ERROR: v4l2 capture: slave not found!\n");
			status = -ENODEV;
		}
		break;
	case V4L2_CID_HUE:
	...
	case V4L2_CID_COLORFX:
		...
		break;
	default:
		os_print("MXC2:ERROR: v4l2 capture: unsupported ioctl: %#08x\n", c->id);
	}
...
//	os_print("MXC2:   clock_curr=mclk=%d\n", ifparm.u.bt656.clock_curr);
...
//	os_print("MXC2:vsync_pol(%d) hsync_pol(%d) ext_vsync(%d)\n", csi_param.Vsync_pol, csi_param.Hsync_pol, csi_param.ext_vsync);

	/* if the capturemode changed, the size bounds will have changed. */
	cam_fmt.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
	vidioc_int_g_fmt_cap(cam->sensor, &cam_fmt);
//	os_print("MXC2:   g_fmt_cap returns widthxheight of input as %d x %d\n",
//			cam_fmt.fmt.pix.width, cam_fmt.fmt.pix.height);
	...
	if (init_defrect) {
		...
		os_print("MXC2:On Open: Input to ipu size is %d x %d\n",
			cam_fmt.fmt.pix.width, cam_fmt.fmt.pix.height);
		os_print("MXC2:End of %s: v2f pix widthxheight %d x %d\n", __func__,
			cam->v2f.fmt.pix.width, cam->v2f.fmt.pix.height);
		os_print("MXC2:End of %s: crop_bounds widthxheight %d x %d\n", __func__,
...
//	os_print("MXC2:%s\n", __func__);
	...
	if (err) {
		os_print("MXC2:%s: vidioc_int_g_parm returned an error %d\n",
			__func__, err);
		goto exit;
	}
...
/*
	os_print("MXC2:   Current capabilities are %x\n",
			currentparm.parm.capture.capability);
	os_print("MXC2:   Current capturemode is %d  change to %d\n",
			currentparm.parm.capture.capturemode,
			parm->parm.capture.capturemode);
	os_print("MXC2:   Current framerate is %d  change to %d\n",
...
*/
	...
	err=0;
	if (err) {
		os_print("MXC2:%s: vidioc_int_s_parm returned an error %d\n",
			__func__, err);
		goto exit;
	}
...
//	os_print("MXC2:%s: %Lx\n", __func__, e);
	switch (e) {
	case V4L2_STD_PAL:
		os_print("MXC2:   Setting standard to PAL %Lx\n", V4L2_STD_PAL);
		cam->standard.id = V4L2_STD_PAL;
		video_index = TV_PAL;
		break;
	case V4L2_STD_NTSC:
		os_print("MXC2:   Setting standard to NTSC %Lx\n",
				V4L2_STD_NTSC);
	...
	default:
		cam->standard.id = V4L2_STD_ALL;
		video_index = TV_NOT_LOCKED;
		os_print("MXC2:ERROR: unrecognized std! %Lx (PAL=%Lx, NTSC=%Lx\n",
	...
	if (cam->device_type == 1) {
		...
		if (cam->standard_autodetect) {
			if (cam->standard.id != tv_fmt.fmt.pix.pixelformat) {
				os_print("MXC2:MVC: mxc_v4l2_g_std: "
					"Changing standard\n");
...
static void read_mipi2_csi2_reg(void);
static void read_mipi2_csi2_reg()
{
	void *info;
	info = mipi2_csi2_get_info();
	mipi2_csi2_dphy_status(info);
	mipi2_csi2_get_error1(info);
	mipi2_csi2_get_error2(info);
}
/*!
...
static int mxc_v4l_dqueue(cam_data *cam, struct v4l2_buffer *buf)
{
	...
	if (!wait_event_interruptible_timeout(cam->enc_queue,
					      cam->enc_counter != 0,
					      50 * HZ)) {
		os_print("MXC2:ERROR: v4l2 capture: mxc_v4l_dqueue timeout "
			"enc_counter %x\n",
		       cam->enc_counter);
		read_mipi2_csi2_reg();
		return -ETIME;
	} else if (signal_pending(current)) {
		os_print("MXC2:ERROR: v4l2 capture: mxc_v4l_dqueue() "
			"interrupt received\n");
		read_mipi2_csi2_reg();
		return -ERESTARTSYS;
	}
	...
	os_print("MXC2:good to go: v4l2 capture: mxc_v4l_dqueue()\n");
	...
	if (frame->buffer.flags & V4L2_BUF_FLAG_DONE) {
		...
		os_print("MXC2:ERROR: v4l2 capture: VIDIOC_DQBUF: "
			"Buffer not filled.\n");
	...
	os_print("MXC2:success to dque: v4l2 capture: mxc_v4l_dqueue()\n");
	read_mipi2_csi2_reg();
...
static int mxc_v4l_open(struct file *file)
{
	...
	if (!cam) {
		os_print("MXC2:%s: %s cam_data not found!\n", __func__, dev->name);
		return -EBADF;
	}
	if (!cam->sensor) {
		os_print("MXC2:%s: %s no sensor ipu%d/csi%d\n",
			__func__, dev->name, cam->ipu_id, cam->csi);
		return -EAGAIN;
	}
	if (cam->sensor->type != v4l2_int_type_slave) {
		os_print("MXC2:%s: %s wrong type ipu%d/csi%d, type=%d/%d\n",
			__func__, dev->name, cam->ipu_id, cam->csi,
	...
	if (!sensor) {
		os_print("MXC2:%s: Internal error, sensor_data is not found!\n",
		       __func__);
		return -EBADF;
	}
	os_print("MXC2:%s: %s ipu%d/csi%d\n", __func__, dev->name,
		cam->ipu_id, cam->csi);
	...
	if (cam->open_count++ == 0) {
		...
		if (test_and_set_bit(csi_bit, &csi_in_use)) {
			os_print("MXC2:%s: %s CSI already in use\n", __func__, dev->name);
			err = -EBUSY;
			cam->open_count = 0;
			goto oops;
		}
		...
		if (!IS_ERR(gpr)) {
			...
			os_print("MXC2:%s: failed to find fsl,imx6q-iomux-gpr regmap\n",
			       __func__);
		}
...
static int mxc_v4l_close(struct file *file)
{
	...
	if (!cam) {
		os_print("MXC2:%s: cam_data not found!\n", __func__);
		return -EBADF;
	}

	if (!cam->sensor) {
		os_print("MXC2:%s: Internal error, camera is not found!\n",
		       __func__);
		return -EBADF;
	}
	...
	if (--cam->open_count == 0) {
		...
		wait_event_interruptible(cam->power_queue,
					 cam->low_power == false);
		os_print("MXC2:mxc_v4l_close: release resource\n");
			    ...
			    loff_t *ppos)
{
	...
	if (!wait_event_interruptible_timeout(cam->still_queue,
					      cam->still_counter != 0,
					      10 * HZ)) {
		os_print("MXC2:ERROR: v4l2 capture: mxc_v4l_read timeout counter %x\n",
		       cam->still_counter);
		err = -ETIME;
		goto exit1;
	}
			    ...
			    unsigned int ioctlnr, void *arg)
{
	...
	case VIDIOC_QUERYCAP: {
		...
		os_print("MXC2:   case VIDIOC_QUERYCAP\n");
	...
	case VIDIOC_G_FMT: {
		...
		os_print("MXC2:   case VIDIOC_G_FMT\n");
	...
	case VIDIOC_S_DEST_CROP: {
		...
		os_print("MXC2:   case VIDIOC_S_DEST_CROP\n");
	...
	case VIDIOC_TRY_FMT: {
		...
		os_print("MXC2:   case VIDIOC_TRY_FMT\n");
		retval = mxc_v4l2_s_fmt(cam, sf, true);
		break;
	}
	case VIDIOC_S_FMT: {
		struct v4l2_format *sf = arg;
		os_print("MXC2:   case VIDIOC_S_FMT\n");
	...
	case VIDIOC_REQBUFS: {
		...
		os_print("MXC2:   case VIDIOC_REQBUFS\n");

		if (req->count > FRAME_NUM) {
			os_print("MXC2:ERROR: v4l2 capture: VIDIOC_REQBUFS: "
			       "not enough buffers\n");
			req->count = FRAME_NUM;
		}
	...
	case VIDIOC_QUERYBUF: {
		...
		int index = buf->index;
		os_print("MXC2:   case VIDIOC_QUERYBUF\n");

		if (buf->type != V4L2_BUF_TYPE_VIDEO_CAPTURE) {
			os_print("MXC2:ERROR: v4l2 capture: "
			       "VIDIOC_QUERYBUFS: "
			       "wrong buffer type\n");
			retval = -EINVAL;
			break;
	...
	case VIDIOC_QBUF: {
		struct v4l2_buffer *buf = arg;
		int index = buf->index;
		os_print("MXC2:   case VIDIOC_QBUF, length=%d\n", buf->length);
		...
		if ((cam->frame[index].buffer.flags & 0x7) ==
			...
			os_print("MXC2:ERROR: v4l2 capture: VIDIOC_QBUF: "
	...
	case VIDIOC_DQBUF: {
		struct v4l2_buffer *buf = arg;
		os_print("MXC2:   case VIDIOC_DQBUF\n");
	...
	case VIDIOC_STREAMON: {
		os_print("MXC2:   case VIDIOC_STREAMON\n");
	...
	case VIDIOC_STREAMOFF: {
		os_print("MXC2:   case VIDIOC_STREAMOFF\n");
	 ...
	 */
	case VIDIOC_G_CTRL: {
		os_print("MXC2:   case VIDIOC_G_CTRL\n");
	 ...
	 */
	case VIDIOC_S_CTRL: {
		os_print("MXC2:   case VIDIOC_S_CTRL\n");
	...
	case VIDIOC_CROPCAP: {
		...
		os_print("MXC2:   case VIDIOC_CROPCAP\n");
	...
	case VIDIOC_G_CROP: {
		...
		os_print("MXC2:   case VIDIOC_G_CROP\n");
	...
	case VIDIOC_S_CROP: {
		...
		os_print("MXC2:   case VIDIOC_S_CROP\n");
		...
		os_print("MXC2:   Cropping Input to ipu size %d x %d\n",
	...
	case VIDIOC_OVERLAY: {
		int *on = arg;
		os_print("MXC2:   VIDIOC_OVERLAY on=%d\n", *on);
		if (*on) {
			cam->overlay_on = true;
	...
	case VIDIOC_G_FBUF: {
		...
		os_print("MXC2:   case VIDIOC_G_FBUF\n");
		*fb = cam->v4l2_fb;
	...
	case VIDIOC_S_FBUF: {
		struct v4l2_framebuffer *fb = arg;
		os_print("MXC2:   case VIDIOC_S_FBUF\n");
		cam->v4l2_fb = *fb;
		break;
	}
	...
	case VIDIOC_G_PARM: {
		...
		os_print("MXC2:   case VIDIOC_G_PARM\n");
		if (cam->sensor)
	...
	case VIDIOC_S_PARM:  {
		struct v4l2_streamparm *parm = arg;
		os_print("MXC2:   case VIDIOC_S_PARM\n");
		if (cam->sensor)
	...
	case VIDIOC_ENUMSTD: {
		...
		os_print("MXC2:   case VIDIOC_ENUMSTD\n");
		if (e->index > 0) {
			retval = -EINVAL;
			break;
		}
	...
	case VIDIOC_G_STD: {
		v4l2_std_id *e = arg;
		os_print("MXC2:   case VIDIOC_G_STD\n");
		if (cam->sensor)
	...
	case VIDIOC_S_STD: {
		v4l2_std_id *e = arg;
		os_print("MXC2:   case VIDIOC_S_STD\n");
	...
	case VIDIOC_ENUMOUTPUT: {
		...
		os_print("MXC2:   case VIDIOC_ENUMOUTPUT\n");
	...
	case VIDIOC_G_OUTPUT: {
		...
		os_print("MXC2:   case VIDIOC_G_OUTPUT\n");
	...
	case VIDIOC_S_OUTPUT: {
		int *p_output_num = arg;
		os_print("MXC2:   case VIDIOC_S_OUTPUT\n");
	...
	case VIDIOC_ENUMINPUT: {
		...
		os_print("MXC2:   case VIDIOC_ENUMINPUT\n");
	...
	case VIDIOC_G_INPUT: {
		int *index = arg;
		os_print("MXC2:   case VIDIOC_G_INPUT\n");
	...
	case VIDIOC_S_INPUT: {
		int index = *(int *)arg;
		os_print("MXC2:   case VIDIOC_S_INPUT(%d)\n", index);
...
/*	case VIDIOC_TRY_FMT: */
	...
	default:
		os_print("MXC2:   case default or not supported\n");
		retval = -EINVAL;
		break;
	}
...
static int mxc_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	os_print("MXC2:%s:pgoff=0x%lx, start=0x%lx, end=0x%lx\n", __func__,
	...
	if (remap_pfn_range(vma, vma->vm_start,
		...
		os_print("MXC2:ERROR: v4l2 capture: mxc_mmap: "
...
static void camera_callback(u32 mask, void *dev)
{
	...
	if (!list_empty(&cam->working_q)) {
		...
		if (done_frame->buffer.flags & V4L2_BUF_FLAG_QUEUED) {
			...
			os_print("MXC2:ERROR: v4l2 capture: camera_callback: "
				"buffer not queued\n");
	}
...
//	csi_id=1;
	...
	if (cam->ipu == NULL) {
		os_print("MXC2:ERROR: v4l2 capture: failed to get ipu\n");
		return -EINVAL;
	} else if (cam->ipu == ERR_PTR(-ENODEV)) {
		os_print("MXC2:ERROR: v4l2 capture: get invalid ipu\n");
		return -ENODEV;
	}
	...
	*(cam->video_dev) = mxc_v4l_template;
	...
	if (cam->dummy_frame.vaddress == 0)
		os_print("MXC2:ERROR: v4l2 capture: Allocate dummy frame "
		       "failed.\n");
...
static int mxc_v4l2_probe(struct platform_device *pdev)
{
	...
	if (cam == NULL) {
		os_print("MXC2:ERROR: v4l2 capture: failed to register camera\n");
		return -1;
	}
	...
	if (video_register_device(cam->video_dev, VFL_TYPE_GRABBER,
		...
		kfree(cam);
		cam = NULL;
		os_print("MXC2:ERROR: v4l2 capture: video_register_device failed\n");
		return -1;
	}

	os_print("MXC2:V4L2 device '%s' on IPU%d_CSI%d registered as %s\n",
		cam->video_dev->name,
...
static int mxc_v4l2_remove(struct platform_device *pdev)
{
	...
	if (cam->open_count) {
		os_print("MXC2:ERROR: v4l2 capture:camera open "
			"-- setting ops to NULL\n");
		return -EBUSY;
	} else {
		...
		os_print("MXC2:V4L2 freeing image input device\n");
	...
	os_print("MXC2:V4L2 unregistering video\n");
	return 0;
}
...
static int mxc_v4l2_master_attach(struct v4l2_int_device *slave)
{
	...
	os_print("MXC2:%s:slave.name = %s, master.name = %s\n", __func__,
		slave->name, slave->u.slave->master->name);

	if (slave == NULL) {
		os_print("MXC2:ERROR: v4l2 capture: slave parameter not valid.\n");
		return -1;
	}

	if ((sdata->ipu_id != cam->ipu_id) || (sdata->csi != cam->csi) || (sdata->mipi_camera != cam->mipi_camera)) {
		os_print("MXC2:%s: ipu(%d:%d)/csi(%d:%d)/mipi(%d:%d) doesn't match\n", __func__,
	...
	if (cam->sensor_index < MXC_SENSOR_NUM) {
		...
		os_print("MXC2:ERROR: v4l2 capture: slave number exceeds "
		       "the maximum.\n");
		return -1;
	}
	...
	os_print("MXC2:%s: ipu%d:/csi%d %s attached %s:%s\n", __func__,
...
void mxc2_camera_common_lock(void)
{
	mutex_lock(&camera_common_mutex);
}
EXPORT_SYMBOL(mxc2_camera_common_lock);

void mxc2_camera_common_unlock(void)
{
	mutex_unlock(&camera_common_mutex);
}
EXPORT_SYMBOL(mxc2_camera_common_unlock);
...
static __init int camera_init(void)
{
	...
	if (err != 0) {
		os_print("MXC2:ERROR: v4l2 capture:camera_init: "
